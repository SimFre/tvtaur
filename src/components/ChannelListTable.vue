<script setup>
    ...
    async function onRowSelect(e) {
        ...
        if (e.type == "stream") {
            ...
            iptv.debug = "null";
            if (e.key in expandedRowGroups.value) {
                delete expandedRowGroups.value[e.key];
            } else {
                expandedRowGroups.value[e.key] = true;
                onRowGroupExpand(e);
            }
        }
    }

    async function onRowGroupExpand(e) {
...
<template>
    <DataTable v-if="iptv.menuNodes.root.length > 0"
        ...
        <Column field="categoryName"
                header="categoryName"></Column>

        <template #groupheader="sp">
            {{ sp.data.categoryName }}
        </template>
    </DataTable>
</template>
